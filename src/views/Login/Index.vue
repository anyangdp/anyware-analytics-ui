...
const onSubmit = async (formEl: FormInstance | undefined) => {
	...
	await formEl.validate(async valid => {
		...
		try {
			loading.value = true
			// 请求登录接口
			let res = await login(loginModel.ruleForm)
			await saveTokenAndInitRoutes(res)
		} catch (e) {
			console.error(e)
		} finally {
			loading.value = false
 ...
 */
const saveTokenAndInitRoutes = async (token: string) => {
	// 缓存 token
		...
		<div class="content">
			<div class="content-inner">
				<div class="login-form-wrapper">
					<div class="login-form-title mb-4">{{ title }}</div>
					<div class="login-form-sub-title"></div>
					<div class="login-form-error-msg"></div>
					<el-form ref="loginFormRef" :model="loginModel.ruleForm" :rules="loginModel.rules" size="large"
									 style="max-width: 800px">
						<el-form-item prop="username">
							<el-input v-model="loginModel.ruleForm.username">
								<template #prefix>
						...
						<!--						</el-form-item>-->
						<el-form-item>
							<el-button type="primary" @click="onSubmit(loginFormRef)" :loading-icon="loadIcon" :loading="loading"
												 style="width: 100%">
								登录
							</el-button>
						</el-form-item>
