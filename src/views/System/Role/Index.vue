...
import { addRole, authorize, delRole, editRole, getRolePage } from '@/api/system/role'
...
const columns = reactive<ColumnProps<RolePageRes>[]>([
	...
	{ prop: 'active', label: '状态', width: 100, tag: true, enum: appActiveDic },
...
const openDrawer = (
	...
	const params: DrawerProps<RolePageRes> = {
		...
		isView: title === '查看',
		row: { ...row },
		api: title === '新增' ? addRole : title === '编辑' ? editRole : authorize,
		...
		<div class="table-box">
			...
			>
				...
				<template #operation="scope">
					...
					<el-button type="primary" link @click="openDrawer('授权', scope.row)">
						<template #icon>
							<pure-icon name="pi-carbon:edit"></pure-icon>
						</template>
						授权
