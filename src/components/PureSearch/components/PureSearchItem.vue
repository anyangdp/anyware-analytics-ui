...
import type { SearchType } from '@/components/PureTable/types/pureTable.type'
import type { SearchFormItemProps } from '@/components/PureSearch/interfaces/pureSearch.interface'
...
/**
 * @description 判断 fieldNames 设置 label && value && children 的 key 值
 */
...
/**
 * @description 接收 enumMap (el 为 select-v2 需单独处理 enumData)
 */
...
/**
 * @description 处理透传的 searchProps (el 为 tree-select、cascader 的时候需要给下默认 label && value && children)
 */
...
/**
 * @description 是否有清除按钮 (当搜索项有默认值时，清除按钮不显示)
 */
...
/**
 * @description 定义 search 类型，不同 search 类型对应不同的 El 组件
 * @return {Record<SearchType, Component>} search 类型组件
 */
const elComponents: Record<SearchType, Component> = {
	input: ElInput,
...
<template>
	<component
		:is="column.search?.render ?? elComponents[column.search?.el as SearchType]"
